...
public class LogAspect {
    ...
    @CrossOrigin
    @Around("within(cn.onlyhi.file.controller..*) && @annotation(logRecordAnnotation)")
    public ResponseEntity<Response> around(ProceedingJoinPoint proceedingJoinPoint, LogRecordAnnotation logRecordAnnotation) throws Throwable {
        ...
        try {
            ...
            //LOGGER.info("LogAspect args={}",JSON.toJSONString(args));
            String token =null;
            Object obj =null;
            if(args.length>0){
                obj = args[0];
            ...
            if (systemLog.getCheckToken()) {
                ...
                //LOGGER.info("LogAspect token==",token);
                //LOGGER.info("LogAspect loginUserCache={}", JSON.toJSONString(loginUserCache));
